...
def caso10():
    ...
    return {
        ...
        "classificacao": {"modalidade": "parametrizar", "representacao": "superficie",
                          "finalidade": "visualizacao"},
        # ACRESCENTADO em M0.5: a borda deste caso passa a ser DECLARADA, e nao deduzida
        # da representacao. Antes, "superficie" implicava borda, o que e falso.
        "topologia": {"borda": "obrigatoria", "estanqueidade": "nao_se_aplica"},
        "malha": stl, "step": None, "requisitos": req, "referencia": None,
        ...
        "espera": {
            ...
            "resultado": "aprovado",
            "extra": "a malha e ABERTA, a borda e declarada obrigatoria, e a varredura "
                     "tem que aprovar mesmo assim",
        },
    }


def familia_casca_fechada(L=30.0, P=20.0, H=10.0):
    """Familia parametrica de SUPERFICIE fechada: casca sem solido, sem borda.

    Existe porque a regra anterior deduzia topologia da representacao e tratava
    "superficie" como sinonimo de casca aberta. Esta e superficie legitima com borda
    PROIBIDA, e a regra antiga a reprovaria por nao ter borda."""
    from build123d import Align, Box, Shell
    return Shell(Box(L, P, H, align=(Align.CENTER, Align.CENTER, Align.MIN)).faces())


def caso11():
    """criar / superficie / visualizacao - casca FECHADA sem solido, borda PROIBIDA.

    Par do caso 10: a mesma representacao, com a topologia oposta. Aqui a estanqueidade e
    exigida pela TOPOLOGIA declarada, e nao pela finalidade, que sozinha nao a pediria.
    Isso e o que prova que topologia e representacao sao decisoes separadas."""
    from build123d import export_stl
    import trimesh
    d = _prep()
    casca = familia_casca_fechada()
    stl = os.path.join(d, "c11_casca_fechada.stl")
    export_stl(casca, stl, tolerance=0.01, angular_tolerance=0.1)
    m = trimesh.load(stl, force="mesh", process=False)
    m.merge_vertices()
    req = _req(os.path.join(d, "c11_req.json"), {
        "peca": stl,
        "requisitos": [
            {"id": "envelope", "tipo": "caixa", "valor": [30.0, 20.0, 10.0], "tol_mm": 0.5},
        ]})
    return {
        "id": "11_casca_fechada_sem_solido",
        "titulo": "casca fechada sem solido, com borda proibida",
        "classificacao": {"modalidade": "criar", "representacao": "superficie",
                          "finalidade": "visualizacao"},
        "topologia": {"borda": "proibida", "orientacao": "exigida",
                      "componentes": 1, "estanqueidade": "exigida"},
        "malha": stl, "step": None, "requisitos": req, "referencia": None,
        "geometria_nao_vazia": {"ok": len(m.faces) > 0, "n_facetas": int(len(m.faces)),
                                "area_mm2": round(float(m.area), 2)},
        "declaracoes": {"encaixe_declarado": False},
        "fatos_da_malha": {"fechada_watertight": bool(m.is_watertight),
                           "n_solidos": 0},
        "espera": {
            "exige": ["intencao", "geometria_nao_vazia", "topologia", "malha_estanque"],
            "nao_exige": ["zero_nao_manifold", "portao_do_fatiador",
                          "envelope_e_particao", "folga_calibrada", "reabre_em_cad"],
            "resultado": "aprovado",
            "extra": ("superficie com borda PROIBIDA: a malha e fechada e isso e o "
                      "requisito, nao o defeito. A estanqueidade e exigida pela "
                      "topologia declarada, nao pela finalidade."),
        },
    }
