...
# --------------------------------------------------------------------------------------
# Geometria da mesa. ACRESCENTADO em M0.4: o filtro anterior exigia extensao positiva nos
# dois eixos, e tres pontos colineares como (0,0), (128,128) e (256,256) tem extensao 256
# nos dois eixos e area ZERO. Poligono que se autointersecta tambem passava.
TOL_AREA = 1e-9


def area_do_poligono(pts):
    """Area assinada pela soma de produtos cruzados. Zero significa degenerada, e o sinal
    diz a orientacao, que aqui nao importa."""
    a = 0.0
    n = len(pts)
    for i in range(n):
        x1, y1 = pts[i]
...
def _segmentos_se_cruzam(p1, p2, p3, p4):
    d1, d2 = _orientacao(p3, p4, p1), _orientacao(p3, p4, p2)
    d3, d4 = _orientacao(p1, p2, p3), _orientacao(p1, p2, p4)
    if ((d1 > 0 > d2) or (d1 < 0 < d2)) and ((d3 > 0 > d4) or (d3 < 0 < d4)):
        return True
    for a, b, c in ((p3, p4, p1), (p3, p4, p2), (p1, p2, p3), (p1, p2, p4)):
        if _orientacao(a, b, c) == 0 and _dentro_da_faixa(a, b, c):
            return True
    return False


def poligono_simples(pts):
    """Devolve (True, None) ou (False, (i, j)) com o par de arestas que se cruza. Arestas
    adjacentes compartilham vertice de proposito e nao contam."""
    n = len(pts)
    for i in range(n):
        for j in range(i + 1, n):
            if j == i + 1 or (i == 0 and j == n - 1):
                continue
            if _segmentos_se_cruzam(pts[i], pts[(i + 1) % n], pts[j], pts[(j + 1) % n]):
                return False, (i, j)
    return True, None
...
def confere_envelope(modelo, perfil_maquina, margem=0.0):
    ...
    if len(pts) >= 3:
        ...
        area = abs(area_do_poligono(pts))
        if not (lx > 0 and ly > 0):
            invalidos.append("printable_area degenerada: extensao %r x %r" % (lx, ly))
            lx = ly = None
        # CORRIGIDO em M0.4: extensao positiva nos dois eixos NAO implica area positiva.
        elif area <= TOL_AREA * max(1.0, lx * ly):
            invalidos.append(
                "printable_area com area nula: %d vertices, extensao %.3f x %.3f mm e "
                "area %.6g mm2. Pontos colineares tem extensao positiva nos dois eixos e "
                "area zero, e por isso a extensao sozinha nao valida a mesa."
                % (len(pts), lx, ly, area))
            lx = ly = None
        else:
            simples, par = poligono_simples(pts)
            if not simples:
                invalidos.append(
                    "printable_area autointersectante: as arestas %d e %d se cruzam. A "
                    "area calculada nao representa a regiao da mesa." % par)
                lx = ly = None
    else:
        lx = ly = None
    env = [lx, ly, h]
    ...
    return {"caixa_da_peca_mm": [round(x, 3) for x in ext],
            ...
            "area_da_mesa_mm2": (round(area, 3) if area is not None else None),
            "n_vertices_da_mesa": len(pts),
            # ACRESCENTADO em M0.4: limitar a conclusao ao que a ferramenta de fato compara
            "base_da_comparacao": (
                "caixa envolvente da mesa contra caixa envolvente da peca. Para mesa NAO "
                "retangular isso nao prova contencao: uma peca pode entrar na caixa da "
                "mesa e nao entrar na mesa. Contencao real exige teste ponto a ponto "
                "contra o poligono, que esta fora do escopo desta ferramenta."),
