...
## M0 executado, 07/09/2026: achados novos, medidos

Registro completo em `../base_congelada/M0_RESULTADO.md`. Os três achados reusáveis:

### O escritor de 3MF não permite classificar a causa da falha

**MEDIDO:** com o caminho de saída ocupado, `Mesher.write` **não** levanta `OSError`.
Levanta `lib3mf.Lib3MF.ELib3MFException` com `error_code` **5**, que a própria biblioteca
chama de `GENERICEXCEPTION`, e o texto vem como *"The specified file could not be
created"*.

Consequência prática: o código numérico da biblioteca **não separa** falha de E/S de
recusa de formato. Quem quiser distinguir falha operacional de reprovação geométrica **não
pode** perguntar à biblioteca, e não deve interpretar a mensagem, porque critério por
frase já falhou aqui uma vez por acento e caixa. A saída é medir o próprio caminho de
saída, antes de chamar o escritor e de novo depois de uma falha.

Eu havia escrito o classificador com a regra "não é `OSError`, então é geométrico", e
declarei no comentário onde ela poderia errar. O aceite mediu e ela errava.

### O 3MF não é portão de graça nesta versão

**MEDIDO:** na variante com tangência (`familia_placa` com `L=80, d=40`), o escritor de
3MF **aceitou** a malha (`portao_3: true`) e quem reprovou foi o portão de malha, por
`arestas_com_mais_de_2_faces`. A afirmação de que o formato fechado recusa o que o STL
aceita não se sustenta neste caso.

### Polígono autointersectante simétrico tem área zero

**MEDIDO:** `["0x0","256x256","256x0","0x256"]` é autointersectante e a soma de produtos
cruzados **cancela**, dando área 0. Um teste que use esse polígono é pego pelo filtro de
área e **nunca exercita** o teste de autointersecção. O par assimétrico
`["0x0","200x200","200x0","0x120"]` tem área 8.000 mm² e cruzamento entre as arestas 0 e
2, e é ele que exercita o teste de verdade.

Corolário geral: quando dois filtros podem pegar o mesmo exemplo, o exemplo não prova
nenhum dos dois. É preciso um caso que só o filtro sob teste consiga pegar.

### Não-manifold e estanqueidade não são independentes

**MEDIDO:** em `trimesh`, `is_watertight` conta faces por aresta, então **qualquer** malha
com aresta não-manifold é não estanque. Não existe malha "limpa em tudo exceto em
não-manifold". Para provar o efeito de um papel de métrica na entrega foi preciso outra
métrica: dois cubos fechados disjuntos são limpos em borda, orientação, estanqueidade e
facetas degeneradas, e falham **só** na contagem de componentes conexos.
