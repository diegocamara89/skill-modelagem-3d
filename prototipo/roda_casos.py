...
APROVADA, REPROVADA, AUSENTE, ERRO, NAO_APLICAVEL = (
    "APROVADA", "REPROVADA", "AUSENTE", "ERRO", "NAO_APLICAVEL")
# ACRESCENTADO em M0.6: falta de MEDIDOR nao e o mesmo que falta de RESULTADO. AUSENTE
# e resultado que nao chegou; NAO_IMPLEMENTADA e o tipo de requisito existir no contrato
# e nao existir medidor para ele. Confundir os dois transforma lacuna de capacidade em
# dispensa legitima, ou em ausencia de evidencia que some do relatorio.
NAO_IMPLEMENTADA = "NAO_IMPLEMENTADA"
ESTADOS_VALIDOS = (APROVADA, REPROVADA, AUSENTE, ERRO, NAO_APLICAVEL, NAO_IMPLEMENTADA)
...
# ACRESCENTADO em M0.3: o tempo limite era constante literal, e por isso a unica forma de
# exercitar o caminho de tempo esgotado seria esperar 900 segundos. Um caminho que nao se
# consegue exercitar nao esta verificado. O padrao continua o mesmo.
TEMPO_LIMITE_S = int(os.environ.get("SKILL3D_TEMPO_LIMITE_S", "900"))
...
def _py(script, args):
    try:
        ...
        return {"_erro": "timeout ao rodar %s apos %s s" % (script, TEMPO_LIMITE_S),
                "_etapa": "execucao_do_verificador", "_tempo_limite_s": TEMPO_LIMITE_S}
    s = r.stdout
    i = s.find("{")
    if i < 0:
...
def ver_topologia(c):
    """ACRESCENTADO em M0.5. Confere a topologia MEDIDA contra a topologia DECLARADA.

    Existe porque a regra anterior deduzia topologia da representacao: "superficie" exigia
    borda, o que reprovaria uma casca fechada sem solido, que tambem e superficie legitima.
    Aqui cada requisito declarado tem uma medida propria, e requisito sem medida sai como
    NAO_IMPLEMENTADA, nunca aprovado por omissao."""
    topo = c.get("topologia")
    if not topo:
        return NAO_APLICAVEL, {"motivo": "o caso nao declara requisitos de topologia"}
    try:
        topo = matriz.valida_topologia(topo)
    except SystemExit as e:
        return ERRO, {"codigo": "E_PERFIL", "motivo": "declaracao de topologia invalida: %s" % e}
    d = _malha(c)
    if _falhou(d):
        return ERRO, d

    medido = {"arestas_abertas": d.get("arestas_abertas"),
              "orientacao_consistente": d.get("orientacao_consistente"),
              "n_componentes_conexos": d.get("n_componentes_conexos"),
              "fechada_watertight": d.get("fechada_watertight")}
    itens, faltam_medida, motivos = {}, [], []

    ab = medido["arestas_abertas"]
    if topo["borda"] == "indiferente":
        itens["borda"] = "NAO_APLICAVEL"
    elif ab is None:
        itens["borda"] = "NAO_IMPLEMENTADA"
        faltam_medida.append("borda")
    else:
        exige = topo["borda"] == "obrigatoria"
        tem = ab > 0
        itens["borda"] = "APROVADA" if tem == exige else "REPROVADA"
        if tem != exige:
            motivos.append("borda declarada %s e a malha tem %d arestas abertas"
                           % (topo["borda"], ab))

    if topo["orientacao"] == "indiferente":
        itens["orientacao"] = "NAO_APLICAVEL"
    elif medido["orientacao_consistente"] is None:
        itens["orientacao"] = "NAO_IMPLEMENTADA"
        faltam_medida.append("orientacao")
    else:
        itens["orientacao"] = ("APROVADA" if medido["orientacao_consistente"]
                               else "REPROVADA")
        if not medido["orientacao_consistente"]:
            motivos.append("orientacao exigida e as normais nao sao consistentes")

    comp = topo["componentes"]
    if comp == "indiferente":
        itens["componentes"] = "NAO_APLICAVEL"
    elif medido["n_componentes_conexos"] is None:
        itens["componentes"] = "NAO_IMPLEMENTADA"
        faltam_medida.append("componentes")
    else:
        itens["componentes"] = ("APROVADA" if medido["n_componentes_conexos"] == comp
                                else "REPROVADA")
        if medido["n_componentes_conexos"] != comp:
            motivos.append("componentes declarados %s e medidos %s"
                           % (comp, medido["n_componentes_conexos"]))

    est = topo["estanqueidade"]
    if est == "indiferente":
        itens["estanqueidade"] = "NAO_APLICAVEL"
    elif est == "nao_se_aplica":
        itens["estanqueidade"] = "NAO_APLICAVEL"
    elif medido["fechada_watertight"] is None:
        itens["estanqueidade"] = "NAO_IMPLEMENTADA"
        faltam_medida.append("estanqueidade")
    else:
        itens["estanqueidade"] = ("APROVADA" if medido["fechada_watertight"]
                                  else "REPROVADA")
        if not medido["fechada_watertight"]:
            motivos.append("estanqueidade exigida e a malha nao e fechada")

    det = {"declarado": topo, "medido": medido, "estado_por_requisito": itens,
           "motivos": motivos, "requisitos_sem_medida": faltam_medida}
    if faltam_medida:
        # lacuna de capacidade nao e dispensa: o requisito decide, e nao ha medidor
        return ERRO, dict(det, codigo="E_CONTRATO_SAIDA",
                          motivo="requisito de topologia declarado sem medidor: %s"
                                 % faltam_medida)
    if any(v == "REPROVADA" for v in itens.values()):
        return REPROVADA, det
    return APROVADA, det
...
def ver_varredura_de_familia(c):
    ...
    # ACRESCENTADO em M0.5: a borda declarada pela familia chega ao executor. Sem isso, o
    # portao de superficie continuaria exigindo borda sempre, deduzindo topologia da
    # representacao.
    if g.get("borda"):
        argv += ["--borda", g["borda"]]
...
def roda(fn):
    ...
    cl = c["classificacao"]
    # ACRESCENTADO em M0.5: a topologia declarada pelo caso chega ao roteamento, e e
    # ela, nao a representacao, que decide as dispensas que dependem de borda.
    rota = matriz.rotear(cl["modalidade"], cl["representacao"], cl["finalidade"],
                         topologia=c.get("topologia"), papeis=c.get("papeis"))
    esp = c["espera"]
    ...
    for nome in rota["exigidas"]:
        ...
        if fnv is None:
            # CORRIGIDO em M0.6: era AUSENTE, e barrava independente do papel. Falta de
            # medidor e NAO_IMPLEMENTADA, e o bloqueio passa a ser qualificado pelo papel.
            resultados[nome] = {
                "estado": NAO_IMPLEMENTADA,
                "detalhe": {"motivo": "nao existe verificador registrado para esta "
                                      "verificacao",
                            "papel": rota["papeis"].get(nome),
                            "nota": "sem medidor nao ha medida. Nada e preenchido com "
                                    "valor favoravel, com zero nem com estimativa."}}
            continue
        try:
            st, det = fnv(c)
        except Exception as e:
    ...
    # CORRIGIDO em M0.6: as listas eram cegas ao papel, entao INFORMATIVA nao existia
    # de fato: qualquer reprovacao barrava. Agora so a DECISIVA entra em reprovadas e em
    # bloqueios; a INFORMATIVA e MEDIDA, registrada e nunca barra.
    papel = rota["papeis"]

    def _decisiva(n):
        return papel.get(n, "DECISIVA") == "DECISIVA"

    reprovadas = [n for n, r in resultados.items()
                  if r["estado"] == REPROVADA and _decisiva(n)]
    reprovadas_informativas = [n for n, r in resultados.items()
                               if r["estado"] == REPROVADA and not _decisiva(n)]
    ausentes = [n for n, r in resultados.items()
                if r["estado"] == AUSENTE and _decisiva(n)]
    erros = [n for n, r in resultados.items() if r["estado"] == ERRO and _decisiva(n)]
    nao_implementadas = [n for n, r in resultados.items()
                         if r["estado"] == NAO_IMPLEMENTADA]
    nao_implementadas_que_barram = [n for n in nao_implementadas if _decisiva(n)]
    ...
    nao_aplic_indevidas = [n for n in nao_aplic
                           if n not in nao_aplicaveis_declaradas and _decisiva(n)]
    bloqueios = sorted(set(ausentes + erros + nao_aplic_indevidas
                           + nao_implementadas_que_barram))
    informativas = {n: {"estado": resultados[n]["estado"],
                        "detalhe": resultados[n]["detalhe"]}
                    for n in resultados if not _decisiva(n)}
    ...
    elif isinstance(esperado, dict) and "barrado_em" in esperado:
        # ACRESCENTADO em M0.6: o vocabulario tinha "aprovado" e um unico caso especial,
        # o da folga. Nao havia como um caso declarar "reprovado EXATAMENTE nesta
        # verificacao", e sem isso o efeito do papel da metrica na entrega nao era
        # expressavel: so se podia esperar aprovacao.
        alvo = sorted(esperado["barrado_em"])
        resultado_ok = sorted(reprovadas) == alvo
        obtido = ("barrado_em %s" % alvo if resultado_ok
                  else "aprovado" if not reprovadas
                  else "reprovado em: " + ", ".join(sorted(reprovadas)))
    else:
    ...
    if c.get("fatos_da_malha"):
        f = c["fatos_da_malha"]
        fechada = bool(f.get("fechada_watertight"))
        # CORRIGIDO em M0.5: este extra assumia que ter fatos_da_malha significa casca
        # ABERTA. E a mesma deducao falsa que M0.5 desfaz, agora no julgamento do caso.
        # Ele passa a seguir a borda DECLARADA; sem declaracao, o comportamento e o de antes.
        borda = (c.get("topologia") or {}).get("borda", "obrigatoria")
        if borda == "proibida":
            extras.append({"nome": "a malha E fechada, como a topologia declarada exige, "
                                   "e o caso passa",
                           "passou": fechada and completo and not reprovadas,
                           "detalhe": f})
        elif borda == "obrigatoria":
            extras.append({"nome": "a malha NAO e fechada e o caso passa mesmo assim",
                           "passou": (not fechada) and completo and not reprovadas,
                           "detalhe": f})
    ...
    return {
        ...
        "reprovadas_em_papel_informativo": reprovadas_informativas,
        "nao_implementadas": nao_implementadas,
        "nao_implementadas_que_barram": nao_implementadas_que_barram,
        "nota_de_papel": ("verificacao INFORMATIVA aparece medida e nao barra. Ela NAO e "
                          "contada como evidencia favoravel: nao entra em reprovadas nem "
                          "em aprovadas, e o motivo dela fica em informativas."),
