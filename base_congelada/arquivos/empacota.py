...
def main():
    ...
    if a.json and a.destino:
        aj = os.path.abspath(a.json)
        ad = os.path.abspath(a.destino)
        dentro = aj == ad or aj.startswith(ad + os.sep)
        if dentro:
            raise SystemExit(
                "recusado: o relatorio (--json %s) fica DENTRO do destino (%s). O relatorio "
                "e gravado depois da auditoria, entao ele invalidaria o que foi auditado. "
                "Grave o relatorio fora do pacote." % (a.json, a.destino))
        if os.path.basename(aj) in PERMITIDOS:
            raise SystemExit(
                "recusado: o relatorio se chama %r, que e nome de arquivo do pacote. Isso "
                "confunde evidencia com codigo entregue." % os.path.basename(aj))
