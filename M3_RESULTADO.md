...
Executado em 08/09/2026. Pacote **2.4.0**, `bl_ferramentas` **1.5.0**.

**Hash do pacote final:** `6fc1727d6e90bdfa65ec3ca80f15c19e1a9e33da9203ff98f33269a36116135b`
...
mesmo valor (`evidencias_finais/AMBIENTE_ISOLADO.json`, `hash_do_pacote_executado`).

**Esta última frase já esteve falsa, e a validação adversarial pegou.** Em 2.2.0 ela
afirmava identidade categórica enquanto o JSON citado trazia o hash de 2.1.0, com 24
arquivos: a versão 2.2.0 diferia apenas por `LICENSE`, `SKILL.md` e `INVENTARIO.json`,
e a ressalva estava no `HASHES.json` em vez de estar aqui. Regra que passa a valer
neste documento: **quando o hash desta linha e o do JSON de execução divergirem, a
divergência aparece aqui**, com o delta, ou o ensaio é refeito. Em 2.3.0 o ensaio foi refeito e os dois hashes coincidiram. Em **2.4.0** a
diferença é `SKILL.md` e `criar_e_parametrizar.md` — texto, sem código — e o
hash executado registrado no JSON é o de 2.3.0, `435d7bb5…`. Fica dito aqui,
como a regra acima manda.
...
| C43 | cada seção medida de um furo é confrontada com o **pedido** | reprodução do caso da validação adversarial, com peça sintética própria | `evidencias_finais/APROVACAO_INDEVIDA_CORRIGIDA.json` | `v_furo` comparava a segunda seção com a **primeira**, e só a primeira com o alvo. Reproduzido com números próprios: bloco 20 × 20 × 10, furo Ø4 de eixo inclinado, primeira seção a **0,19** do alvo e segunda a **0,29**, tolerância **0,20**, desvio entre seções 0,10 → **APROVADA**. Depois da correção: **REPROVADA**, código 1, com `erro_de_posicao_da_segunda_mm 0.29`. O desvio entre seções continua sendo conferido, porque pega inclinação e conicidade | **ATENDIDO** |
| C44 | volume só é declarado confiável se os componentes não se sobrepõem | duas cascas fechadas sobrepostas na mesma malha | `evidencias_finais/APROVACAO_INDEVIDA_CORRIGIDA.json` | `volume_confiavel` era `fechada and not negativo`. Dois cubos de lado 10 deslocados 5 em x: **2000 mm³** declarados confiáveis, união real **1500**, excesso de **500** (33%), com «Pode seguir». Depois: `volume_confiavel false`, sobreposição de 500 mm³ **medida** comparando a soma das cascas com o volume da união, e nomeada em `aviso_de_volume` e em `acao`. A sobreposição **não** entra em `motivos_de_reprovacao`: a malha não está quebrada, e misturar as duas coisas seria o erro que este portão existe para evitar | **ATENDIDO** |
| C45 | a preservação de região exige **área**, não só conjunto de posições | face removida mantendo os vértices | `produto/scripts/bl_ferramentas.py`, `compara_regiao_protegida` | a função declarava, no próprio campo `limite`, que conjunto de posições idêntico com contagem de faces diferente é «retesselação, não alteração da superfície». Falso: apagando uma face de um tetraedro os quatro vértices continuam nas faces vizinhas, o conjunto fica idêntico e **3,4641 mm²** desaparecem. Agora a captura guarda **área e número de faces**, e o veredito é `PRESERVADA` só com as duas medidas iguais, `ALTERADA` quando a área muda, e `INDETERMINADO` quando a captura vem de versão que não media área | **ATENDIDO** |
| C46 | entrada não é destruída por colisão de caminho | mesmo arquivo como cena e como resultado | `evidencias_finais/CONTROLES_DA_VALIDACAO.json` | `roda()` apagava o resultado **antes** de conferir a cena: com o mesmo caminho nos dois papéis, a cena era **destruída** e o retorno dizia «cena não encontrada», apresentando a perda como ausência. Agora a entrada é conferida antes de qualquer remoção e a colisão é recusada: `COLISAO_DE_CAMINHO`, com o arquivo **intacto** (mesmo sha256 antes e depois) | **ATENDIDO** |
| C47 | o código de saída do processo entra no veredito | lançador substituto que escreve relatório favorável e termina em 7 | `evidencias_finais/CONTROLES_DA_VALIDACAO.json` | o código era **registrado** e não decidia: processo terminando em 7 com `{"veredito_global":"ATENDIDO"}` no disco saía `OK` e código **0**, inclusive com `--exigir`. E o próprio produto documenta que código não zero é sinal confiável de falha — medir um sinal e não usá-lo é pior que não medir. Agora: `PROCESSO_COM_ERRO` e código não zero na CLI | **ATENDIDO** |
| C48 | integridade confere **hash**, e o manifesto exige **cobertura** | bytes acrescentados sem atualizar o manifesto, e manifesto vazio | `evidencias_finais/CONTROLES_DA_VALIDACAO.json` | `--verificar` era apresentado como o teste de integridade e **não conferia hash nenhum**; e a resolução do manifesto aprovava `{"arquivos": {}}` com `n_declarados: 0, resolve: true`. Ausência de divergência numa lista vazia não é integridade — é a mesma armadilha do «0 achados» sobre padrão nenhum, que este projeto já havia corrigido na varredura. Agora `--verificar` confere os hashes da origem e `resolve` exige **cobertura exata** dos arquivos que devem ser identificados; nos dois ataques o código é 1 e nada é montado | **ATENDIDO** |
| C49 | a árvore de trabalho não é confundida com o artefato | contagem de bytecode na origem | `evidencias_finais/empacotamento.json`, campo `bytecode_na_arvore_de_origem` | a pasta de trabalho contém `.pyc` com o caminho absoluto do autor — criados por rodar os próprios scripts — e a enumeração de origem os pulava, de modo que `--verificar` dizia «limpo» sobre uma árvore que os continha. **A lista de permissão funciona:** o destino montado tem **zero**. O relatório passou a **nomear** os arquivos encontrados na árvore e a dizer que distribuir é montar o destino, nunca copiar a pasta | **ATENDIDO** |
...
**C17 fica PENDENTE sobre 2.4.0**, e é o que falta para o pacote poder ser chamado de
